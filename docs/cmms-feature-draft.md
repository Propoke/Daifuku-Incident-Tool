...
- **Phase 2 — feature-complete**: ~~reporting/dashboards~~ done, ~~service contract/SLA tracking~~ done (measurement only, no auto-escalation), ~~safety & compliance module~~ done (permit-to-work/LOTO + incident/near-miss reporting; calibration/inspection due-dates reuse the existing PMSchedule mechanism rather than a new one), ~~helpdesk self-service~~ done for internal users (`/report-issue/`, `/my-tickets/` — any authenticated employee, not just Technician+ roles), ~~field service dispatch~~ done for internal staff (`WorkOrder.scheduled_start/scheduled_end`, `/dispatch/` board grouped by team, view-only — no drag-drop rescheduling), ~~customer-facing portal~~ done (`portal` app, local username/password accounts via `assets.CustomerContact`, scoped by `Asset.owner_customer` — completely separate from the internal Entra/site-scoping system; see the security note below), ~~mobile technician app~~ done as an installable PWA at `/app/` (see `docs/mobile-app-backlog.md` — offline support and real-device camera testing are the known remaining gaps, both flagged before it was built).

  **Portal security note**: local accounts mean this app now owns password security for external users - no rate limiting/lockout on login attempts, no forced password rotation, no self-service password reset flow (an internal Admin/Asset Manager sets/resets passwords via `/admin/`). Fine for a small number of known contacts; revisit (rate limiting at minimum, ideally passwordless email links) before opening this to many customers or exposing it to the internet.
...
- **Status: implemented, site is the single access-control axis** — `assets.access` (query-scoping at the application layer, not DB-level row security) restricts Asset/WorkOrder/PMSchedule/StockLocation/StockLevel/StockReservation to a user's assigned site(s), enforced in both the Django admin and the mobile API. Sites are assignable to individual users or to a whole Group. An "OEM" group and the existing "Admin" group bypass it entirely (see everything); anyone else with no site assignment sees nothing (fail closed) until assigned one. Every Asset requires a Terminal (and therefore a Site) regardless of who owns it — ownership (`Asset.owner_customer`) is a separate, purely identifying attribute (billing/warranty/contract), deliberately **not** a second access-control axis; a customer-owned asset is scoped exactly like an internal one, by the site it physically sits at. Explicit product decision, not a gap: there is no plan for customer-based scoping unless a customer-portal use case actually requires it later.
...
- **SLA/escalation rules engine**: §6/§4 mention SLA management and escalation — this tends to grow into its own subsystem (breach detection, notification timing, business-hours-aware clocks). Worth scoping explicitly rather than treating as "a field on the ticket." **Status: breach detection/measurement built** (`workorders.sla.get_sla_status()`, `ServiceContract.sla_response_hours`/`sla_resolution_hours`, surfaced in the WorkOrder admin and the reporting dashboard) — response/resolution targets against actual timestamps, no business-hours awareness (treats all hours as elapsed, not working hours). **Not built**: notifications, auto-escalation, or any business-hours clock — this is the measurement half of the subsystem, not the alerting half.
